(function () {
    ...
    class App {
        ...
        setupNavigation() {
            document.addEventListener('click', (e) => {
                ...
                // Статьи уже генерируются как полноценные HTML-страницы:
                // /<category>/<id>/index.html. Не перехватываем такой переход SPA.
                // Браузер получает готовую страницу напрямую, без fetch() Markdown.
                const cleanPath = href.split(/[?#]/)[0].replace(/^\/+|\/+$/g, '');
                const routeParts = cleanPath ? cleanPath.split('/') : [];
                const articleCategory = routeParts.length === 2 ? getCategoryFromSlug(routeParts[0]) : null;
                if (articleCategory && routeParts[1]) {
    ...
    <div class="share-buttons" id="share-buttons-${article.id}"></div>
            ...
            // Контент уже сгенерирован в HTML на этапе build.
            // Никаких runtime-запросов к Markdown больше не требуется.
